...
Document upload and scanning are what `GET /api/{rec}/config` reports in `features.document_upload` and `features.document_scan`; the wizard reads them and never infers availability. They are off unless the deployment sets both `DPA_SIGNED` and `OPENAI_API_KEY`, because scanning sends identity documents to the extraction provider. Off, the feature is absent rather than disabled: no upload control, no placeholder, and no step requires a document.
